...
( function( $ ) {
	...
	/* ------------ arrows in primary menu ------------- */
		$( '.advisor-nav-logo-header .sub-menu .menu-item-has-children, .advisor-nav-logo-header .children > .page_item_has_children' )
	...
	/* ------------ for no-avatar in comments ------------- */
		if( ! $( '.vcard img' ).is( '.avatar' ) ) {
			...
			$( '.advisor-comment .advisor-clear, .pingback .advisor-clear' ).css( { 'border-top' : '1px solid #e1e1e1', 'margin-top' : '10px' } );
		}
	...
	/* ------------ focus on search in widgets ------------- */
		...
		search.each( function() {
			if ( '' != $( this ).val() ) {
				$( this ).css( 'width', '77.8%' ).parent().css( { 'width':'77.8%', 'border-color':'#d1e2c0', 'background-color':'#fff' } );
			}
			$( this ).focus( function() {
			 	$( this ).css( 'width', '77.8%' ).parent().css( { 'width':'77.8%', 'border-color':'#d1e2c0', 'background-color':'#fff' } );
			} );
